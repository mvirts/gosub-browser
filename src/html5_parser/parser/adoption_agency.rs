...
impl<'a> Html5Parser<'a> {
    ...
    pub fn run_adoption_agency(&mut self, token: &Token) {
        ...
        loop {
                ...
                match self.active_formatting_elements[idx] {
                    ...
                    ActiveElement::NodeId(node_id) => {
                        ...
                        if node.is_special() {
                            ...
                            furthest_block_children = self.document.get_node_by_id(furthest_block_id)
            ...
            let common_ancestor = *self.open_elements.get(common_ancestor_idx).expect("common ancestor not found");
            ...
            let mut last_node_id = *self.open_elements.get(last_node_idx).expect("last node not found");
            ...
            // Step 4.15
            let new_element = Node::new_element(formatting_element_name.as_str(), formatting_element_attributes, HTML_NAMESPACE);
